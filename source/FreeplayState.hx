...
class FreeplayState extends MusicBeatState
{
	...
	private var AllPossibleSongs:Array<String> = ["story", "extra", "joke", "covers"];
	...
	override function create()
	{
		...
		CurrentSongIcon = new FlxSprite(0,0).loadGraphic(Paths.image('packs/' + (AllPossibleSongs[CurrentPack].toLowerCase())));
	...
	public function LoadProperPack()
	{
		...
		{
			case 'story':
				addWeek(['Blu'], 0, ['barren']);
				addWeek(['Anillo'], 1, ['lenzo']);
				addWeek(['Remorseless'], 2, ['morrow']);
				addWeek(['Overdrive'], 3, ['cbarren']);
				addWeek(['Amber'], 4, ['lenzai']);
				addWeek(['Divinity'], 5, ['mordon']);
				addWeek(['Ultimatum'], 6, ['mordon']);
			case 'extra':
				addWeek(['Harmony'], 7, ['dave']);
				addWeek(["Rockin'"], 8, ['bambi']);
				addWeek(['Golden'], 10, ['tristan']);
				addWeek(['Empowered'], 9, ['bf']);
			case 'joke':
				addWeek(['get real'], 7, ['dave-angey']);
			case 'covers':
				addWeek(['Heart of Gold'], 10, ['gbarren']);
				addWeek(['Recursed'], 11, ['shiftBarren']);
		}
	}
	...
	public function UpdatePackSelection(change:Int)
	{
		...
		CurrentSongIcon.loadGraphic(Paths.image('packs/' + (AllPossibleSongs[CurrentPack].toLowerCase())));
	}
